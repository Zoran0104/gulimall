    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/gulimallorder/pmscategorybrandrelation/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.brandId = data.pmsCategoryBrandRelation.brandId
                this.dataForm.catelogId = data.pmsCategoryBrandRelation.catelogId
                this.dataForm.brandName = data.pmsCategoryBrandRelation.brandName
                this.dataForm.catelogName = data.pmsCategoryBrandRelation.catelogName
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/gulimallorder/pmscategorybrandrelation/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
