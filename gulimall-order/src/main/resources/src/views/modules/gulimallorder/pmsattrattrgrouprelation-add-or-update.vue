    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/gulimallorder/pmsattrattrgrouprelation/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.attrId = data.pmsAttrAttrgroupRelation.attrId
                this.dataForm.attrGroupId = data.pmsAttrAttrgroupRelation.attrGroupId
                this.dataForm.attrSort = data.pmsAttrAttrgroupRelation.attrSort
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/gulimallorder/pmsattrattrgrouprelation/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
