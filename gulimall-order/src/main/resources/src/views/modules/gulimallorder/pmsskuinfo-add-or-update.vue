    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.skuId) {
            this.$http({
              url: this.$http.adornUrl(`/gulimallorder/pmsskuinfo/info/${this.dataForm.skuId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.spuId = data.pmsSkuInfo.spuId
                this.dataForm.skuName = data.pmsSkuInfo.skuName
                this.dataForm.skuDesc = data.pmsSkuInfo.skuDesc
                this.dataForm.catalogId = data.pmsSkuInfo.catalogId
                this.dataForm.brandId = data.pmsSkuInfo.brandId
                this.dataForm.skuDefaultImg = data.pmsSkuInfo.skuDefaultImg
                this.dataForm.skuTitle = data.pmsSkuInfo.skuTitle
                this.dataForm.skuSubtitle = data.pmsSkuInfo.skuSubtitle
                this.dataForm.price = data.pmsSkuInfo.price
                this.dataForm.saleCount = data.pmsSkuInfo.saleCount
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/gulimallorder/pmsskuinfo/${!this.dataForm.skuId ? 'save' : 'update'}`),
              method: 'post',
