    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/gulimallorder/pmsspuimages/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.spuId = data.pmsSpuImages.spuId
                this.dataForm.imgName = data.pmsSpuImages.imgName
                this.dataForm.imgUrl = data.pmsSpuImages.imgUrl
                this.dataForm.imgSort = data.pmsSpuImages.imgSort
                this.dataForm.defaultImg = data.pmsSpuImages.defaultImg
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/gulimallorder/pmsspuimages/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
