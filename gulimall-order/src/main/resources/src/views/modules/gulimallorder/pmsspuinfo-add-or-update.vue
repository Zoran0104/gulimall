    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/gulimallorder/pmsspuinfo/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.spuName = data.pmsSpuInfo.spuName
                this.dataForm.spuDescription = data.pmsSpuInfo.spuDescription
                this.dataForm.catalogId = data.pmsSpuInfo.catalogId
                this.dataForm.brandId = data.pmsSpuInfo.brandId
                this.dataForm.weight = data.pmsSpuInfo.weight
                this.dataForm.publishStatus = data.pmsSpuInfo.publishStatus
                this.dataForm.createTime = data.pmsSpuInfo.createTime
                this.dataForm.updateTime = data.pmsSpuInfo.updateTime
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/gulimallorder/pmsspuinfo/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
