    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/gulimallorder/pmsskuimages/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.skuId = data.pmsSkuImages.skuId
                this.dataForm.imgUrl = data.pmsSkuImages.imgUrl
                this.dataForm.imgSort = data.pmsSkuImages.imgSort
                this.dataForm.defaultImg = data.pmsSkuImages.defaultImg
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/gulimallorder/pmsskuimages/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
